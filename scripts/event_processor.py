...
class SessionFSM:
    ...
    def process_sessions(self, df):
        ...
        for _, row in df.iterrows():
            ...
            if self.fsm.current_state == 'IDLE' and action == self.START_EVENT:
                self._start_session(action, timestamp)

            elif self.fsm.current_state == 'SESSION_ACTIVE':
                # Exhibition
                if action.startswith(self.EXHIBIT_PREFIX) or action.startswith(self.MENUEXHIBIT_PREFIX):
                ...
                if action.startswith(self.CTRL_PREFIX) or action.startswith(self.IMAGE_ZOOM):
                    self.current_event["actions"].append((action, timestamp))

                elif action.startswith(self.UI_CLOSE):
                    self._exit_exhibit(timestamp)

    def _start_session(self, action, timestamp):
        ...
        start_event = {"state": self.fsm.current_state, "type": "START", "exhibit": None, "exhibit_id": None, "start_time": timestamp, "end_time": timestamp, "actions": [(action, timestamp)]}
    ...
    def _enter_exhibit(self, action, timestamp):
        ...
        self.current_event = {"state": self.fsm.current_state, "type": "CONTENT", "exhibit": action, "exhibit_id": None, "start_time": timestamp, "end_time": None, "actions": []}
    ...
    def _track_exploration(self, action, timestamp):
        ...
        if self.exploration_event:
            ...
            self.exploration_event = {"type": self.fsm.current_state, "type": "MAP", "exhibit": None, "exhibit_id": None, "start_time": timestamp, "end_time": None, "actions": []}
    ...
    def _close_session(self, action, timestamp):
        ...
        if self.current_session:
            ...
            end_event = {"type": "END", "exhibit": None, "exhibit_id": None, "start_time": timestamp, "end_time": timestamp, "actions": [(action, timestamp)]}
        ...
        for session_id, session in enumerate(self.sessions):
            ...
            for event in session["events"]:
                ...
                session_data.append([
                    ...
                    event["type"], event["exhibit"], exhibit_id, event_start, event_end, event_duration, actions_count
                ])
